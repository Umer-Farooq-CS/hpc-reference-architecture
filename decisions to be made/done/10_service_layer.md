...
Tenants need a way to spin up GPU-backed Jupyter notebooks on demand.
*   **Decision:** Confirm the use of **KubeFlow Notebooks** as the JNaaS backend. This leverages native Kubernetes CRDs and integrates cleanly with the KAI Scheduler for GPU allocation.

### 2. ModelaaS Inference Engine
Tenants need a fast, OpenAI-compatible API to serve their fine-tuned LLMs.
*   **Decision:** Decide between **vLLM** (open-source, PagedAttention, fastest performance), **NVIDIA NIM** (requires AI Enterprise license, pre-optimized), or **Triton** (better for traditional non-LLM models) as the default inference backend integrated with KServe.

### 3. MLOps Registry
Tenants need a place to track training runs, log metrics, and store model weights.
*   **Decision:** Confirm the adoption of **MLflow** for experiment tracking and model versioning, using your MinIO (Cold Storage) tier as the artifact backend.

### 4. Tenant Frontend (SSP)
Tenants need a web portal to request resources and view their quota.
*   **Decision:** Decide whether you will expose the built-in **Rafay SSP / KubeFlow Central Dashboard** directly to tenants, or if you will build a **custom React portal** that abstracts those tools via API calls for a fully branded experience.

---
*Next: See `11_standard_architecture.md` for the final recommended reference architecture combining all best tools.*
